                              ...
                              std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        {
            LogPrint("mempool", "replacing tx %s with %s for %s ELC additional fees, %d delta bytes\n",
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    ...
    CAmount nSubsidy = 1000 * COIN; //Changing initial award
...
{
	return false; //++ Disabling initial block donwload
    const CChainParams& chainParams = Params();
    LOCK(cs_main);
