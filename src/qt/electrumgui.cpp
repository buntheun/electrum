...
void ElectrumGUI::createActions()
{
    ...
    aboutAction = new QAction(platformStyle->TextColorIcon(":/icons/about"), tr("&About Electrum Core"), this);
    aboutAction->setStatusTip(tr("Show information about Electrum Core"));
    ...
    optionsAction->setStatusTip(tr("Modify configuration options for Electrum Core"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the Electrum Core help message to get a list with possible Electrum command-line options"));
...
void ElectrumGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
    ...
    QString toolTip = tr("Electrum Core client") + " " + networkStyle->getTitleAddText();
...
void ElectrumGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress)
{
    if(!clientModel)
        return;
    ...
    menu(0)
{
    ...
    QList<ElectrumUnits::Unit> units = ElectrumUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    Q_FOREACH (const ElectrumUnits::Unit unit, units)
    {
        max_width = qMax(max_width, fm.width(ElectrumUnits::name(unit)));
    }
...
void UnitDisplayStatusBarControl::createContextMenu()
{
    ...
    {
        QAction *menuAction = new QAction(QString(ElectrumUnits::name(u)), this);
...
{
    setText(ElectrumUnits::name(newUnits));
}
