...
CElectrumAddress GetAccountAddress(string strAccount, bool bForceNew=false)
{
    ...
    return CElectrumAddress(account.vchPubKey.GetID());
}
...
UniValue sendtoaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 2 || params.size() > 5)
        throw runtime_error(
            "sendtoaddress \"electrumaddress\" amount ( \"comment\" \"comment-to\" subtractfeefromamount )\n"
...
UniValue encryptwallet(const UniValue& params, bool fHelp)
{
    ...
    StartShutdown();
    return "wallet encrypted; Electrum server stopping, restart to run with encrypted wallet. The keypool has been flushed, you need to make a new backup.";
}
...
UniValue listunspent(const UniValue& params, bool fHelp)
{
    ...
    BOOST_FOREACH(const COutput& out, vecOutputs) {
        ...
        CTxDestination address;
        if (ExtractDestination(out.tx->vout[out.i].scriptPubKey, address)) {
            entry.push_back(Pair("address", CElectrumAddress(address).ToString()));
