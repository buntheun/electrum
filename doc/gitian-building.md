...
*Setup instructions for a Gitian build of Electrum using a Debian VM or physical system.*
...
- [Building Electrum](#building-electrum)
...
git clone https://github.com/electrum/electrum
```
...
in the electrum repository under 'Fetch and build inputs' to install sources which require
...
To build Electrum (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the electrum repository.
...
Output from `gbuild` will look something like
    ...
    From https://github.com/electrum/electrum
...
For example:
```bash
URL=https://github.com/laanwj/electrum.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit electrum=${COMMIT} --url electrum=${URL} ../electrum/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit electrum=${COMMIT} --url electrum=${URL} ../electrum/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit electrum=${COMMIT} --url electrum=${URL} ../electrum/contrib/gitian-descriptors/gitian-osx.yml
```
...
ELCPATH=/some/root/path/electrum.git
SIGPATH=/some/root/path/electrum-detached-sigs.git

./bin/gbuild --url electrum=${ELCPATH},signature=${SIGPATH} ../electrum/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
[electrum/gitian.sigs](https://github.com/electrum/gitian.sigs/) repository, or if that's not possible create a pull
